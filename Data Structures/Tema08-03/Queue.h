...
public:
	...
	*/
	...
	void jump(const T pringao, Queue<T> &colegas) {
		...
		Nodo *nodo1 = _prim;
		//Buscamos al pringao
		while (nodo1 != NULL && !found) {
			if (nodo1->_elem == pringao)
				found = true;
			else
				nodo1 = nodo1->_sig;
		}
		//Si hemos encontrado al pringao, colamos a los colegas detrás
		if (found) {
			colegas._ult->_sig = nodo1->_sig;
			nodo1->_sig = colegas._prim;
			colegas._prim = NULL;
			colegas._ult = NULL;
		}
	}
